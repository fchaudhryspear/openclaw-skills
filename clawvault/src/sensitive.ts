...
import { SensitiveDataDetection, SensitiveDataType, SensitivityLevel } from './types';

export class SensitiveDataDetector {
  private patterns: Array<{ type: SensitiveDataType['type']; regex: RegExp; severity: SensitiveDataType['severity'] }> = [
    // PII - High severity
  ...
  private hasOverlap(start: number, end: number, ranges: Array<[number, number]>): boolean {
    return ranges.some(([rStart, rEnd]) => 
  ...
  private redactContent(content: string, detectedTypes: SensitiveDataType[]): string {
  ...
  determineSensitivityLevel(detection: SensitiveDataDetection): SensitivityLevel {
    if (!detection.hasSensitiveData) return 'public';
    ...
    if (hasHigh) return 'restricted';
    if (hasMedium) return 'confidential';
    return 'internal';
  }

  sanitize(content: string, targetLevel: SensitivityLevel = 'public'): string {
    const detection = this.detect(content);
    
    if (!detection.hasSensitiveData) return content;
