...
import { ConfidenceScore, MemoryEntry, MemoryMetadata } from './types';

export class ConfidenceScorer {
  private readonly VERIFICATION_WEIGHT = 0.3;
  private readonly SOURCE_WEIGHT = 0.25;
  private readonly CONSISTENCY_WEIGHT = 0.2;
  private readonly AGE_WEIGHT = 0.15;
  private readonly ACCESS_WEIGHT = 0.1;
  ...
  calculate(entry: MemoryEntry): ConfidenceScore {
    ...
    const overall = 
      factors.sourceReliability * this.SOURCE_WEIGHT +
  ...
  private scoreSourceReliability(metadata: MemoryMetadata): number {
  ...
  private scoreConsistency(entry: MemoryEntry): number {
    ...
    let score = 0.5;
    if (hasRelationships) score += 0.2;
    if (hasSemanticLayer) score += 0.15;
    if (hasEntities) score += 0.15;
    
    return Math.min(score, 1.0);
  }

  private scoreVerification(entry: MemoryEntry): number {
    ...
    const semanticScore = Math.min(
      (entry.semanticLayer.entities.length + entry.semanticLayer.concepts.length) / 10,
      0.5
    );
  ...
  private scoreAge(timestamp: number): number {
    const age = Date.now() - timestamp;
    const oneDay = 24 * 60 * 60 * 1000;
    const oneMonth = 30 * oneDay;
    
    if (age < oneDay) return 1.0;
    if (age < oneWeek()) return 0.9;
    if (age < oneMonth) return 0.8;
    if (age < 3 * oneMonth) return 0.7;
    if (age < 6 * oneMonth) return 0.6;
    return 0.5;
    
    function oneWeek() { return 7 * oneDay; }
  }

  private scoreAccessPatterns(metadata: MemoryMetadata): number {
    ...
    if (recency < oneDay) score += 0.3;
    else if (recency < 7 * oneDay) score += 0.2;
    else if (recency < 30 * oneDay) score += 0.1;
